        ...
        {iconData.map((icon, index) => (
          ...
          >
            <div className="w-16 h-16 flex items-center justify-center border-2 border-purple-300 rounded-xl p-3 transition-all duration-300  hover:scale-110"> 
              <Image src={icon.src} alt={icon.name} width={40} height={40} className="transition-all duration-300 hover:scale-125" /> {/* ✅ Icon zoom effect */}
            </div>
            <p className="text-purple-600 text-sm font-medium">{icon.name}</p>
          </div>
        ))}
      </div>

      {/* Explore Button */}
      <button className="mt-10 bg-purple-600 hover:bg-purple-700 text-white py-3 px-6 rounded-full text-sm font-semibold shadow-md transition">
